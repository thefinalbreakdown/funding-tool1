# Rebuilt app.py with Plotly + custom tooltips + all logic (ROI, APR, exchange support)
...
if file1 and file2:
    ...
    def normalize(df, exchange, key):
        ...
        if exchange == "Manual":
            ...
            interval = st.number_input(f"Manual interval (hours) for {key}", min_value=1, value=8, step=1, key=key)
        elif exchange == "Bybit":
            df['time'] = pd.to_datetime(df[df.columns[0]])
            df['funding_rate'] = df[df.columns[2]].astype(float) * 100
            interval = int(df.get('Funding Interval', 8))
            interval = st.number_input(f"Detected interval: {interval}h — override?", min_value=1, value=interval, step=1, key=key)
        elif exchange == "WooX":
            df['time'] = pd.to_datetime(df[df.columns[0]])
            ...
            interval = int(df['Funding Interval'].str.replace('h','').iloc[0])
            interval = st.number_input(f"Detected interval: {interval}h — override?", min_value=1, value=interval, step=1, key=key)
            ...
            interval = int(df['Funding Interval'].astype(str).str.extract(r'(\d+)')[0].iloc[0])
            interval = st.number_input(f"Detected interval: {interval}h — override?", min_value=1, value=interval, step=1, key=key)
        else:
    ...
    df_long, interval_long = normalize(df_long, exch_long, 'long')
    df_short, interval_short = normalize(df_short, exch_short, 'short')
